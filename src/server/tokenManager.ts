...
function isTokenOlderThan24Hours(token: TokenData): boolean {
...
export function cleanupOldTokens(ws: WebSocket): void {
  ...
  try {
    ...
    if (
      ...
      console.error(
        "Cannot clean up old tokens: tokenStore not properly initialized"
      );
      return;
    }
    ...
    console.log(
      `Unsubscribing from ${oldTokenMints.length} tokens older than 24 hours`
    );
    ...
    for (let i = 0; i < oldTokenMints.length; i += CHUNK_SIZE) {
      ...
      console.log(
        `Unsubscribed from trades for ${chunk.length} old tokens (batch ${
    ...
    global.pumpFunState.tokenStore = tokens.filter(
      (token) => !isTokenOlderThan24Hours(token)
    );

    console.log(
      `Completed cleanup. Removed ${oldTokenMints.length} old tokens. New token store length: ${global.pumpFunState.tokenStore.length}`
    );
  } catch (error) {
...
export function subscribeToTokenTrades(ws: WebSocket, mint: string): void {
  if (!mint || !ws || ws.readyState !== WebSocket.OPEN) {
  ...
  try {
    ...
    if (global.pumpFunState?.subscribedTokens?.has(mint)) {
...
export function updateTokenTraderTiers(
  ...
  if (!global.pumpFunState || !Array.isArray(global.pumpFunState.tokenStore)) {
    console.log(
      "Global state or tokenStore not initialized, skipping trader tier update"
    );
    return;
  }
  ...
  if (tokenIndex >= 0) {
    ...
    const existingTraderIndex = token.traderTiers.traders.findIndex(
      (t) => t.address === traderAddress
    );
      ...
      if (existingTrader.profitTier !== profitTier) {
        ...
        switch (existingTrader.profitTier) {
          ...
          case ProfitTier.TIER5:
            ...
            break;
          case ProfitTier.TIER6:
            token.traderTiers.tier6.count--;
            break;
          case ProfitTier.TIER7:
            token.traderTiers.tier7.count--;
            break;
          case ProfitTier.TIER8:
...
export function updateTierStats(mint: string): void {
  if (
    !global.pumpFunState ||
    !Array.isArray(global.pumpFunState.tokenStore) ||
    ...
    console.log(
      "Global state, tokenStore, or traderStore not properly initialized, skipping trader tier update"
    );
    return;
  }
  ...
  const tokenIndex = pumpFunState.tokenStore.findIndex(
    (token) => token && token.mint === mint
  );
  ...
  allTiers.forEach((currentTier) => {
    ...
    const tradersInTier = traderTiers.traders.filter(
      (trader) => trader.profitTier === currentTier
    );
  ...
  const actualTokenIndex = pumpFunState.tokenActualStore.findIndex(
    (t) => t && t.mint === mint
  );
  if (actualTokenIndex >= 0) {
