...
import { subscribeToTokenTrades, cleanupOldTokens } from "./tokenManager";
import {
...
import { subscribeToAll } from "./subscriptionManager";
...
export async function connectToExternalWS(wss: WebSocketServer) {
  ...
  console.log("Current tokenStore state:", {
    tokenStoreExists: !!global.pumpFunState?.tokenStore,
    tokenStoreLength: global.pumpFunState?.tokenStore?.length || 0,
    isArray: Array.isArray(global.pumpFunState?.tokenStore),
  });
  ...
  try {
    ...
    externalWs = new WebSocket("wss://pumpportal.fun/api/data");
    ...
    externalWs.onopen = async () => {
      ...
      if (externalWs && externalWs.readyState === WebSocket.OPEN) {
        ...
        try {
          ...
          if (global.pumpFunState?.subscribedTokens) {
            global.pumpFunState.subscribedTokens.clear();
          }

          // Clean up old tokens first
          if (externalWs && externalWs.readyState === WebSocket.OPEN) {
            cleanupOldTokens(externalWs);
          }
          ...
          if (externalWs && externalWs.readyState === WebSocket.OPEN) {
            ...
            await subscribeToAll(externalWs);
          }
        } catch (err) {
    ...
    externalWs.onmessage = async (event) => {
      try {
        ...
        const data = safeJsonParse(event.data.toString());
        if (!data) {
          return;
        }
        ...
        if (
          ...
          console.error(
            "Global state not properly initialized for handling token trade"
          );
          return;
        }

        const globalTokens = global.pumpFunState.tokenStore;
        const tokenIndex = globalTokens.findIndex(
          (token) => token && token.mint === data.mint
        );
        ...
        if (data.traderPublicKey && data.txType && data.mint) {
          ...
          if (isBuy || isSell) {
            const token = global.pumpFunState?.tokenStore.find(
              (t) => t && t.mint === data.mint
            );
            if (!token) {
              console.warn(
                `Token ${data.mint} not found in tokenStore for trader ${traderAddress}`
              );
            }
            // Calculate devTokensSold including the current transaction (if developer sell)
            const validTokenAmount =
              data.tokenAmount > 0 ? data.tokenAmount : 0;

            // Calculate profit if it's a sell transaction
            let profit = 0;
            if (isSell && global.pumpFunState.traderStore.has(traderAddress)) {
              const traderData =
                global.pumpFunState.traderStore.get(traderAddress)!;
              const buyTxs = traderData.transactions.filter(
                (tx) => tx.mint === data.mint && tx.txType === "buy"
              );
              ...
              if (buyTxs.length > 0 && data.tokenAmount > 0) {
                ...
                for (const tx of buyTxs) {
                  if (totalTokensBought >= data.tokenAmount) break;
                  if (!tx.tokenAmount || tx.tokenAmount <= 0) {
                    console.warn(
                      `Skipping invalid buy transaction for trader ${traderAddress}, mint ${data.mint}: tokenAmount=${tx.tokenAmount}`
                    );
                    continue;
                  }
                  const tokensUsed = Math.min(
                    tx.tokenAmount,
                    data.tokenAmount - totalTokensBought
                  );
                ...
                if (
                  totalTokensBought >= data.tokenAmount &&
                  totalTokensBought > 0
                ) {
                  ...
                  console.warn(
                    `Not enough tokens bought for sell by ${traderAddress}: needed ${data.tokenAmount}, have ${totalTokensBought}`
                  );
                  profit = 0;
                }
                ...
                recordTraderTransaction(
                  ...
                  traderAddress,
                  data.mint,
                  data.txType as "buy" | "sell",
                  data.solAmount || 0,
                  validTokenAmount,
                  isSell ? profit : undefined,
                  data.signature
                );
              } else {
                profit = 0;
        ...
        if (data.uri && data.mint) {
          try {
            ...
            const globalTokens = global.pumpFunState.tokenStore;
            const isDuplicate = globalTokens.some(
              (token) => token && token.mint === tokenData.mint
            );
            ...
            if (!isDuplicate) {
              ...
              if (externalWs && externalWs.readyState === WebSocket.OPEN) {
                subscribeToTokenTrades(externalWs, tokenData.mint);
              }
              ...
              wss.clients.forEach((client) => {
                if (client.readyState === WebSocket.OPEN) {
                  try {
                    client.send(
                      JSON.stringify({ type: "newToken", token: tokenData })
                    );
                  } catch (err) {
            ...
            console.error(
              `Error fetching ${data.uri} or processing token metadata: ${error}`
            );
          }
        }
        // Handle token trades
          ...
          if (tokenIndex >= 0) {
            ...
            const currentPriceUsd = data.marketCapSol
              ? (data.marketCapSol * global.pumpFunState.solanaPrice) /
                TOTAL_SUPPLY
              : 0;
            const topPriceUsd = token.topPriceUsd ?? currentPriceUsd;
            const updatedTopPriceUsd =
              currentPriceUsd > topPriceUsd ? currentPriceUsd : topPriceUsd;
            ...
            const topMcap = token.topMcap ?? currentMarketCap;
            ...
            const updatedToken = {
              ...
              marketCapSol:
                data.marketCapSol !== undefined
                  ? data.marketCapSol
                  : token.marketCapSol,
              marketCapUsd:
                data.marketCapUsd !== undefined
                  ? data.marketCapSol * global.pumpFunState.solanaPrice
                  : (token.marketCapSol ?? 0) * global.pumpFunState.solanaPrice,
              vSolInBondingCurve:
                data.vSolInBondingCurve !== undefined
            ...
            if (Array.isArray(global.pumpFunState.tokenActualStore)) {
              ...
              const globalActualIndex = globalActualTokens.findIndex(
                (token) => token && token.mint === data.mint
              );
            ...
            if (externalWs && externalWs.readyState === WebSocket.OPEN) {
              try {
                ...
                console.error(
                  `Error requesting info for unknown token ${data.mint}:`,
                  err
                );
              }
            }
        ...
        // Handle responses to token info requests
          ...
          try {
            ...
            if (!isDuplicate) {
              console.log(
                "Adding new token from info request to store:",
                tokenData.mint
              );
              global.pumpFunState.tokenStore = [tokenData, ...globalTokens];
        ...
        if (data.mint) {
          ...
          const tokenIndexCurrent = globalTokensCurrent.findIndex(
            (token) => token && token.mint === data.mint
          );
          ...
          if (tokenIndexCurrent >= 0) {
            ...
            if (
              (token.marketCapUsd ?? 0) - (token.marketCapTrack ?? 0) >
              2000
            ) {
              const updatedToken = {
                ...token,
              ...
              if (
                globalActualIndex > 11 ||
                globalActualIndex == -1 ||
                globalActualTokens.length < 12
              ) {
                if (globalActualIndex >= 0) {
                  global.pumpFunState.tokenActualStore.splice(
                    globalActualIndex,
                    1
                  );
                }
                ...
                wss.clients.forEach((client) => {
                  if (client.readyState === WebSocket.OPEN) {
                    try {
                      ...
                      console.error(
                        "Error sending token trade to client:",
                        err
                      );
                    }
                  }
                ...
                console.log(
                  `updating token ${data.mint} in actual store, Token Index: ${globalActualIndex}`
                );
...
export function setupExternalWS(wss: WebSocketServer, server: Server) {
  ...
  const pingInterval = setInterval(() => {
    ...
    if (externalWs && externalWs.readyState === WebSocket.OPEN) {
      ...
      connectToExternalWS(wss);
    } else if (externalWs.readyState !== WebSocket.CONNECTING) {
      console.log(
        "External WebSocket not connected (state:",
        externalWs.readyState,
  ...
  const traderCleanupInterval = setInterval(() => {
    if (externalWs && externalWs.readyState === WebSocket.OPEN) {
      ...
      cleanupInactiveTraders(externalWs);
    } else {
